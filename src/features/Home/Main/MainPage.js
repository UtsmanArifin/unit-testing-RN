...
const MainPage = () => {
    ...
    const {onLogout, isNameExist} = useAuth();
    const [name, setName] = useState('')
    // const [pinVisible, setPinVisible] = useState(false);
    const route = useRoute();
    useEffect(()=>{
        const onGetUserNameProfile = async () => {
            try {
                const resp = await isNameExist();
                console.log(resp);
                if (resp) {
                    setName(resp)
                }
                ...
                <ScrollView>
                    <View style={{flex: 1, margin: theme.spacing.s}}>
                        ...
                        <View style={{marginHorizontal: theme.spacing.s}}>
                            <FormButton label={'Logout ' + name} onClick={() => {
